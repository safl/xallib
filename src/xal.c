...
xal_close(struct xal *xal)
{
	...
	if (xal->dev) xnvme_buf_free(xal->dev, xal->buf);
...
int
retreive_total_entries(char *path) 
{
	struct stat sb;
	DIR *d;
	struct dirent *entry;
	...
	err = stat(path, &sb);
	if (err) {
		XAL_DEBUG("FAILED: stat(); errno(%d)", errno);
		return -1;
	}
	...
	while ((entry = readdir(d)) != NULL) {
		...
		if (entry->d_type == DT_DIR) {
			char subpath[sizeof(path) + 1 + sizeof(entry->d_name) + 1];
			int children;

			snprintf(subpath, sizeof(subpath), "%s/%s", path, entry->d_name);
			children = retreive_total_entries(subpath);
...
int
xal_open_mountpoint(char *path, struct xal **xal)
{
	struct xal *cand;
	struct stat sb;
	int err = 0;
	...
	nallocated = retreive_total_entries(path);

	cand = calloc(1, sizeof(*cand));
	cand->mountpoint = path;
	...
	cand->sb.agblocks = 1; // assume one maybe ??
...
process_inode_dir_mounted_extents(struct xal *xal, char *path, struct xal_inode *inode)
{
	...
	while ((entry = readdir(d)) != NULL) {
		...
		char dentry_path[sizeof(path) + 1 + sizeof(dentry->name) + 1];
...
/*
 * So the idea is to create a fiemap object with a fm_extents array of
 * size 0, then set the "extent_count" to be 0, so the ioctl knows that
 * there is room for no extents.
 * Then, the ioctl sets the "mapped_extents" integer to the amount of 
 * extents that there are, so we reallocate the fiemap to be of the 
 * right size, and then run the ioctl again with "extent_count" set to
 * the right size too, such that all the extents are read into the 
 * object.
 */
struct fiemap *
read_fiemap(int fd)
{
	...
	fiemap->fm_length = ~0;  // logical length of mapping which userspace wants (in) // THIS SHOULD BE FILE SIZE

	if (ioctl(fd, FS_IOC_FIEMAP, fiemap) < 0) {
		XAL_DEBUG("FAILED: fiemap ioctl() 1");
		goto failed;
	}
	...
	if (!fiemap) {
		XAL_DEBUG("FAILED: fiemap realloc()");
		goto failed;
	}
	...
	if (ioctl(fd, FS_IOC_FIEMAP, fiemap) < 0) {
		XAL_DEBUG("FAILED: fiemap ioctl() 2");
		goto failed;
	}
...
process_inode_file_mounted_extents(struct xal *xal, char *path, struct xal_inode *inode)
{
	...
	if (fiemap->fm_mapped_extents > 0) {
		...
		if (err) {
			XAL_DEBUG("FAILED: xal_pool_claim_extents()");
			goto failed;
		}

		struct xal_extents *extents = &inode->content.extents;
...
process_ino_fiemap(struct xal *xal, char *path, struct xal_inode *self)
{
	...
	XAL_DEBUG("ENTER process_ino_fiemap()");
	...
	self->ino = sb.st_ino;
	self->size = sb.st_size; // unsure whether these are 1:1
	...
	switch(self->ftype) {
		...
		case XAL_ODF_DIR3_FT_REG_FILE:
			...
			break;
		default:
			XAL_DEBUG("FAILED: Unsupported file-type.");
			return -ENOSYS;
	}

	XAL_DEBUG("EXIT process_ino_fiemap()");
...
int
xal_index_from_mountpoint(struct xal *xal)
{
	int err; 
...
xal_index(struct xal *xal)
{
	...
	if (xal->mountpoint) {
		return xal_index_from_mountpoint(xal);
	}
