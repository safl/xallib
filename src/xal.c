...
xal_open_with_mountpoint(struct xnvme_dev *dev, struct xal **xal)
{
	...
	cand->mountpoint = calloc(XAL_PATH_MAXLEN, sizeof(char));
	if (!cand->mountpoint) {
		XAL_DEBUG("FAILED: calloc()");
		return -errno;
	}

	err = retrieve_mountpoint(ident->uri, cand->mountpoint);
	if (err) {
		XAL_DEBUG("FAILED: retrieve_mountpoint()");
		return err;
	}

	XAL_DEBUG("INFO: dev(%s) mounted at path(%s)", ident->uri, cand->mountpoint);
...
xal_dinodes_retrieve(struct xal *xal)
{
	...
	if (xal->mountpoint) {
		XAL_DEBUG("SKIPPED: xal opened with mountpoint, xal_dinodes_retrieve() skipped")
		return 0;
	}
...
int
index_from_mountpoint(struct xal *xal)
{
	int err;
...
xal_index(struct xal *xal)
{
	...
	if (xal->mountpoint) {
		return index_from_mountpoint(xal);
	}
